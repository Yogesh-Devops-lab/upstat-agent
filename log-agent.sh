...
# Parse parameters or read environment variables
API_KEY="${UPSTAT_API_KEY}"
BACKEND_URL="${UPSTAT_BACKEND_URL:-https://upstat.yogeshramadoss.cloud}"
LOG_TYPE="${UPSTAT_LOG_TYPE:-docker}"
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --log-type) LOG_TYPE="$2"; shift ;;
    esac
    shift
done

if [ -z "$API_KEY" ]; then
    echo "Error: --api-key or UPSTAT_API_KEY environment variable is required."
    exit 1
fi

echo "=========================================="
echo "   Installing UpStat Log Monitoring Agent "
echo "=========================================="

# Create directory
mkdir -p /etc/upstat-log-agent

# Write config parameters
echo "UPSTAT_API_KEY=$API_KEY" > /etc/upstat-log-agent/config
echo "UPSTAT_BACKEND_URL=$BACKEND_URL" >> /etc/upstat-log-agent/config
echo "UPSTAT_LOG_TYPE=$LOG_TYPE" >> /etc/upstat-log-agent/config

# Create python agent
cat << 'EOF' > /etc/upstat-log-agent/agent.py
import json
import time
import os
import subprocess
...
chmod +x /etc/upstat-log-agent/agent.py

# Create systemd service
cat << 'EOF' > /etc/systemd/system/upstat-log-agent.service
[Unit]
Description=UpStat Log Monitoring Agent
After=network.target docker.service
Requires=docker.service

[Service]
Type=simple
ExecStart=/usr/bin/python3 -u /etc/upstat-log-agent/agent.py
Restart=always
RestartSec=10
...
systemctl enable upstat-log-agent.service
systemctl restart upstat-log-agent.service

echo "=========================================="
echo "   UpStat Log Agent installed successfully! "
echo "=========================================="
